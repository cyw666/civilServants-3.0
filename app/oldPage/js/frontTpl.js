 ...
 * 20161015 xia pagemodule使用中，容器内模块识别
 * 20161015 xia 关于tmpt的样式未涉及到modulebase 与 pagemodule 。modulebase的初始目的是为了后台效果显示使用；
 * 20161017 xia 添加延迟模块
 ...
 * @returns {type}
 */
function frontTpl(configoptions) {
    var _this = this;
    ...
    var _baseFun = {
        ...
        allready: function (hasReady, tmptId) {
            if (hasReady) {
                ...
                if ($(".pagemodule:not(.rendered)").length === 0 && _event.ready.all.length > 0) { //局部渲染不触发allReady事件；
        ...
        endLoad: function (tmptid) {
            var $tmpt = $m("#" + tmptid);
            // $tmpt.not(":has(.pagemodule)").find(".content > * , .tmpt-load > *").css({ "visibility": "visible" }); //多个not pagemodule是防止容器渲染完成，而里面没有完成造成提前显示；
        ...
        //无内容异步加载模块，(无pagemodule修饰，所以不影响全部渲染完成事件等，相对独立)初始没有还没有内容的模块，进行加载内容或延迟加载内容，内容加载后则形成正常模块并进行初始化
        initDelay: function () {
    ...
    var build = function ($module) {           // routevalue ->'moduleid=干教资讯&page=1&rows=6&sort=Id&order=desc&ids=9&wordLimt=28,moduleid=最新资讯&page=1&rows=6&sort=Id&order=desc&ids=7&wordLimt=28'
        ...
        if ($module.find(_setting.containerSelector).length > 0) {
            ...
            if ($module.find(".module-title").length > 0) {
                ...
                $modules = $module.find(".module-content>.pagemodule");
                if ($modules.length > 0) {
                ...
                $modules = $module.find(".pagemodule");
                if ($modules.length > 0) {
     ...
     */
    var init = function () {
        var tmpts = $(".pagemodule[tmpt-url],.pagemodule[tmpt-data]");
        if (tmpts.length > 0) {
            tmpts.each(function () {
                _baseFun.init($(this));
            });
    ...
    */
    ...
    this.openWindow = function (options) {
        ...
        //var a = function () {
        //    if (tab.$ && tab.$(".pagemodule:not(.rendered)").length === 0) {
...
function InitLoginUsername(value) {
    var outputHtml = "";
    if (value) {
        outputHtml += "<input id=\"txtAccount\" name=\"Account\" class=\"inp_txt\" type=\"text\" value=\"" + value + "\" onfocus=\"if (this.value == '请输入用户名') { this.value = '' }\" , onblur=\"if (this.value == ''|| this.value == 'null') { this.value = '请输入用户名' }\" />";
    }
    else {
        outputHtml += "<input id=\"txtAccount\" name=\"Account\" class=\"inp_txt\" type=\"text\" value=\"" +
        "请输入用户名" +
        "\" onfocus=\"if (this.value == '请输入用户名') { this.value = '' }\" , onblur=\"if (this.value == ''|| this.value == 'null') { this.value = '请输入用户名' }\" />";
    }
    return outputHtml;
}
function InitLoginPassword(value) {
    var outputHtml = "";
    if (value) {
        outputHtml += "<input id=\"txtAccount\" name=\"Account\" class=\"inp_txt\" type=\"text\" value=\"" +
            value +
            "\" onfocus=\"if (this.value == '请输入密码') { this.value = '' }\" , onblur=\"if (this.value == ''|| this.value == 'null') { this.value = '请输入密码' }\" />";
    }
    else {
        outputHtml += "<input id=\"txtAccount\" name=\"Account\" class=\"inp_txt\" type=\"text\" value=\"" +
            "请输入密码" +
...
function InitLoginRememberFlag(value) {
    var outputHtml = "";
    ...
    outputHtml += "<input id=\"ckRemember\" name=\"ckRemember\" value=\"" +
            isRemember +
            "\" type=\"hidden\" />";
    return outputHtml;
}
